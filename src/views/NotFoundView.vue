<script setup>
import { notFoundViewTexts } from '@/constants'
...
<template>
  <section class="text-center flex flex-col justify-center items-center h-96">
    ...
    <h1 class="text-6xl font-bold mb-4">{{ notFoundViewTexts.notFound }}</h1>
    <p class="text-xl mb-5">{{ notFoundViewTexts.notFoundText }}</p>
    <RouterLink to="/" class="text-white bg-green-700 hover:bg-green-900 rounded-md px-3 py-2 mt-4"
      >{{ notFoundViewTexts.goBack }}
    </RouterLink>
  </section>
</template>
